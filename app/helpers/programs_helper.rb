          ...
          programs_for_school.each do |program|
            buf_tr = content_tag :tr do
              buf_td1 = content_tag :td do
                content_tag :a, Instrument.find(program.instrument_id).name, :class => "instrument-options", :href => "#", "data-type" => "select2", "data-pk" => "#{program.id}", "data-url" => "/programs/save_instruments", "data-source" => "/programs/get_instruments.json", "data-value" => "1", "data-title" => "Select Instrument"
              end
