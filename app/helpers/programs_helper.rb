          ...
          programs_for_school.each do |program|
            buf_tr = content_tag :tr do
              ...
              buf_td2 = content_tag :td do
                content_tag :a, ProgramType.find(program.program_type_id).abbrev, :class => "course-type-options", :href => "#", "data-type" => "select2", "data-pk" => "#{program.id}", "data-url" => "/programs_page/save_program_type", "data-source" => "/programs_page/get_program_types.json", "data-value" => "1", "data-title" => "Select Course Type"
              end
